...
namespace XenkoCodeTestBenchmarks.UIPanels
{
    public class GridCacheProperties : GridBase
    {
        ...
        protected override Vector3 MeasureOverride(Vector3 availableSizeWithoutMargins)
        {
            ...
            for (var dim = 0; dim < 3; dim++)
            {
                ...
                var elementToStripDefinitions = this.elementToStripDefinitions[dim];
                var stripIndexToNoStarElements = this.stripIndexToNoStarElements[dim];
            ...
            for (var dim = 0; dim < 3; dim++)
            {
                ...
                var oneStarSize = 0f;
                foreach (var kv in partialStarElementToStripDefinitions[dim])
                {
                    var element = kv.Key;
        ...
        private void CalculateStarStripSize(Vector3 finalSizeWithoutMargins)
        {
            ...
            for (var dim = 0; dim < 3; dim++)
            {
                var starDefinitions = dimToStarDefinitions[dim];
                starDefinitionsCopy.Clear();
                starDefinitionsCopy.AddRange(starDefinitions);
        ...
        private void RebuildMeasureCacheData()
        {
            ...
            for (var dim = 0; dim < 3; dim++)
            {
                ...
                var stripIndexToNoStarElements = this.stripIndexToNoStarElements[dim];
                var stripDefinitions = this.stripDefinitions[dim];
            ...
            foreach (var child in VisualChildrenCollection)
            {
                ...
                for (var dim = 0; dim < 3; ++dim)
                {
                    ...
                    {
                        var partialStarDefinitions = partialStarElementToStripDefinitions[dim][child];
                        for (var i = childPositionStart; i < childPositionEndExcl; i++)
            ...
            for (var dim = 0; dim < 3; ++dim)
            {
                var starDefinitions = this.dimToStarDefinitions[dim];
        ...
        private void RebuildStripPositionCacheData()
        {
            ...
            for (var dim = 0; dim < 3; dim++)
            {
                var cachedStripIndexToStripPosition = this.cachedStripIndexToStripPosition[dim];
                var stripDefinitions = this.stripDefinitions[dim];
